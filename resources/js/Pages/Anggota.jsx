...
const Anggota = ({ anggota, struktur, flash, kepengurusanlab, tahunKepengurusan, filters }) => {
  ...
  const fetchActiveMembersFromPrevious = async (kepengurusanLabId) => {
    try {
      const response = await fetch(`/anggota/active-members-from-previous?kepengurusan_lab_id=${kepengurusanLabId}`);
      const data = await response.json();
      
      if (data.success) {
        setAnggotaSebelumnya(data.active_members);
      } else {
        toast.error('Gagal mengambil data anggota dari kepengurusan sebelumnya');
      }
    } catch (error) {
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="mb-4">
                  ...
                  <div className="text-xs text-blue-600 mt-1">
                    Password akan otomatis menggunakan NIM/NIDN/NIP ini
                  </div>
            ...
            <form onSubmit={handleTransfer}>
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <select
                    value={transferForm.data.kepengurusan_lab_id}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => transferForm.setData('struktur_id', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                    required
                  >
              ...
              {transferForm.data.kepengurusan_lab_id && (
                <div className="mb-6">
                  <h4 className="font-medium text-gray-700 mb-3">Pilih Anggota yang Akan Ditransfer:</h4>
                  <div className="max-h-60 overflow-y-auto border border-gray-200 rounded-md p-3">
                    {anggotaSebelumnya && anggotaSebelumnya.length > 0 ? (
                      anggotaSebelumnya.map((item) => (
                        <label key={item.id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded">
                          <input
                            type="checkbox"
                            value={item.id}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              const currentSelected = transferForm.data.anggota_dipilih;
                              
                              if (checked) {
                                transferForm.setData('anggota_dipilih', [...currentSelected, item.id]);
                              } else {
                                transferForm.setData('anggota_dipilih', currentSelected.filter(id => id !== item.id));
                              }
                            }}
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-500">
                              {item.nomor_induk || item.email} - {item.struktur} (Kepengurusan {item.kepengurusan_tahun})
                            </div>
                          </div>
                        </label>
                      ))
                      ...
                      <p className="text-gray-500 text-center py-4">
                        {transferForm.data.kepengurusan_lab_id ? 'Tidak ada anggota aktif di kepengurusan yang dipilih' : 'Pilih kepengurusan terlebih dahulu'}
                      </p>
                    )}
                  </div>
                </div>
